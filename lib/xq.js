...
(function() {
  var ALWAYS, FIN, INI, NVA, SERIAL, X, alwaysResolver, cnt, doneResolver, failResolver, filterResolver, forEachResolver, makeResolver, nextTick, safeCall, stepWith, thenResolver, unitFe, unitFx, unwrap,
  ...
  filterResolver = function(fx, fe, v, isError, cb) {
    var filterCb;
    if (isError) {
      return false;
    }
    filterCb = function(vb, isError, ended) {
      if (isError) {
        return cb(vb, isError, ended);
      } else {
        if (vb === NVA) {
          return cb(NVA, false, ended);
        } else if (vb) {
          return cb(v, false, ended);
        } else {
          return cb(NVA, false, ended);
        }
      }
  ...
  X.prototype.filter = stepWith('filter', filterResolver);
